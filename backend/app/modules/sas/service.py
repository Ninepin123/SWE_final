...
_UNLIMITED_DEPARTMENTS = {"不限", "不限科系", "ALL"}


def _allowed_departments(value: str | None) -> list[str]:
    """解析獎學金的科系限制，回傳允許的科系清單（空清單＝不限）。

    SMS 以 JSON 陣列字串儲存（例如 '["資工系","電機系"]'，複選的結果）；
    亦相容舊資料的分隔符純文字（例如 '資工系,電機系'）與「不限科系」語意字串。
    """
    if not value or value.strip() in _UNLIMITED_DEPARTMENTS:
        return []
    text = value.strip()
    ...
    # 過濾掉代表「不限」的元素（例如舊資料存成 ["不限科系"]）
    return [item for item in items if item not in _UNLIMITED_DEPARTMENTS]
        ...
        result.append(
            {
                ...
                "required_documents": [],
                "require_recommendation": scholarship.require_recommendation,
...
def submit_application(db: Session, student: User, application_id: int) -> dict:
    ...
    if scholarship.require_recommendation:
        recommendation_count = db.scalar(
            select(func.count(Recommendation.rec_id)).where(
                Recommendation.application_id == application_id,
            )
        ) or 0
        if recommendation_count == 0:
            raise HTTPException(status_code=400, detail="請先邀請至少一位老師撰寫推薦信")
    app.status = "UNDER_REVIEW"
